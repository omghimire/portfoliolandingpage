...
for path in sorted(glob.glob('climate/climate-vlog*.html')):
    ...
    if magic in text:
        ...
        if not match:
            ...
            text = text[:match.start(2)] + new_content + text[match.end(2):]
            Path(path).write_text(text, encoding='utf-8')
            status = 'UPDATED'
    content_only = re.search(r'<section class="vlog-content justified">(.*?)</section>', text, re.S)
    plain = re.sub(r'<.*?>', '', content_only.group(1) if content_only else '')
    words = len(re.findall(r'\w+', plain))
    log_lines.append(f'{path},{status},{words}')
Path('climate_update_log.txt').write_text('\n'.join(log_lines), encoding='utf-8')
print('DONE')
